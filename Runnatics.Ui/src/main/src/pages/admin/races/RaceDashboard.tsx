...
const PIE_COLORS = ['#2196F3', '#4CAF50', '#f44336'];
...
export const RaceDashboard: React.FC<RaceDashboardProps> = ({
  ...
  useEffect(() => {
    if (eventId && raceId) {
      DashboardService.getRaceStats(eventId, raceId)
        .then(setDashStats)
        .catch(() => {});
    }
  }, [eventId, raceId]);
      ...
      {dashStats && (
        <Box sx={{ mb: 3 }}>
          <Grid container spacing={2} sx={{ mb: 2 }}>
            {[
              { label: 'Total Participants', value: dashStats.totalParticipants, color: '#1976d2' },
              { label: 'Started', value: dashStats.totalStarted, color: '#2196F3' },
              { label: 'Finished', value: dashStats.totalFinished, color: '#4CAF50' },
              { label: 'DNF / Not Started', value: dashStats.totalDnfOrNotStarted, color: '#f44336' },
            ].map((s) => (
          ...
          <Card>
            <CardContent>
              <Typography variant="subtitle1" fontWeight={700} gutterBottom>Race Progress</Typography>
              <ResponsiveContainer width="100%" height={260}>
                <PieChart>
                  <Pie
                    data={[
                      { name: 'Started', value: dashStats.totalStarted },
                      { name: 'Finished', value: dashStats.totalFinished },
                      { name: 'DNF/Not Started', value: dashStats.totalDnfOrNotStarted },
                    ]}
                    cx="50%"
                    cy="50%"
                    outerRadius={90}
